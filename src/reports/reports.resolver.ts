...
export class ReportsResolver {
  ...
  async createReport(
    ...
    @Args('createReportData') reportData: CreateReportInput,
  ): Promise<Report> {
  ...
  async createCommentReport(
    @CurrentUser() user: User,
    @Args('createCommentReportData') commentData: CreateCommentInput,
  ): Promise<Report> {
  ...
  async createProgressReport(
    @CurrentUser() user: User,
    @Args('createProgressReportData') progressData: CreateProgressInput,
  ): Promise<Report> {
  ...
  async medias(@Parent() report: Report) {
    return this.mediasService.getMedias({
      _ids: report.medias.map((m) => m._id.toString()),
    });
  }
  ...
  async upVotes(@Parent() report: Report) {
    return this.usersService.getUsers({
      _ids: report.upVotes.map((uv) => uv._id.toString()),
    });
  }
  ...
  async comments(@Parent() report: Report) {
    return this.commentsService.getComments({
      _ids: report.comments.map((c) => c._id.toString()),
    });
  }

  @ResolveField(() => [Progress], { nullable: true })
  async progresses(@Parent() report: Report) {
    return this.progressesService.getProgresses({
      _ids: report.comments.map((c) => c._id.toString()),
    });
  }
  ...
  async tags(@Parent() report: Report) {
    return this.tagsService.getTags({
      _ids: report.tags.map((t) => t._id.toString()),
    });
  }
  ...
  async updateStatusReport(
    ...
    @Args('updateStatusReportData') updateStatusReport: UpdateStatusReportInput,
  ): Promise<Report> {
    await this.adminsService.verifyAdminRole(admin._id.toString());
    return await this.reportsService.updateStatusReport(
      admin,
      updateStatusReport,
    );
  }
  ...
  async upVoteReport(
    @CurrentUser() user: User,
    @Args() getReportArgs: GetReportArgs,
  ): Promise<Report> {
    const report = await this.reportsService.upVoteReport(
      user,
      getReportArgs._id,
    );
    return report;
  ...
  async updateReviewReport(
    @CurrentUser() user: User,
    @Args('updateReviewReportData') updateReviewReport: UpdateReviewReportInput,
  ): Promise<Report> {
    return await this.reportsService.updateReviewReport(
      user,
      updateReviewReport,
    );
  }
